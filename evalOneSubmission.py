...
def parseData(d4Df, forecastDf, diagLabels):
  ...
  invalidResultReturn = (None, None, None, None, None,
  ...
  for s in range(nrSubj):
    ...
    if currSubjData.shape[0] == 0:
      print('WARNING: Subject RID %s missing from user forecasts' %
            d4Df['RID'].iloc[s])
      invalidFlag = True
      continue
    ...
    if currSubjData.shape[0] < 5 * 12:
      print('WARNING: Missing forecast months for subject with RID %s' %
            d4Df['RID'].iloc[s])
      invalidFlag = True
      continue
    ...
    timeDiffsScanCog = [d4Df['CognitiveAssessmentDate'].iloc[s] -
                        d for d in currSubjData['Forecast Date']]
    ...
    timeDiffsScanMri = [d4Df['ScanDate'].iloc[s] -
                        d for d in currSubjData['Forecast Date']]
  ...
  return zipTrueLabelAndProbs, hardEstimClassFilt, trueDiagFilt


def getSub(d4Df, forecastDf):
  """
  ...
  forecastDf['Forecast Date'] = [datetime.strptime(
      x, '%Y-%m') for x in forecastDf['Forecast Date']]
  if isinstance(d4Df['Diagnosis'].iloc[0], str):
    d4Df['CognitiveAssessmentDate'] = [datetime.strptime(
        x, '%Y-%m-%d') for x in d4Df['CognitiveAssessmentDate']]
    d4Df['ScanDate'] = [datetime.strptime(
        x, '%Y-%m-%d') for x in d4Df['ScanDate']]
    mapping = {'CN': 0, 'MCI': 1, 'AD': 2}
    d4Df.replace({'Diagnosis': mapping}, inplace=True)

  diagLabels = ['CN', 'MCI', 'AD']

  zipTrueLabelAndProbs, hardEstimClass, trueDiagFilt = \
...
def evalOneSub(d4Df, forecastDf):
  ...
  # adasWES = np.sum(adasCoeffs * np.abs(adasEstim -
  #                                      trueADASFilt)) / np.sum(adasCoeffs)

  # ventsCoeffs = 1.0 / (ventriclesEstimUp - ventriclesEstimLo)
  # ventsWES = np.sum(ventsCoeffs * np.abs(ventriclesEstim -
  #                                        trueVentsFilt)) / np.sum(ventsCoeffs)
...
if __name__ == "__main__":
  ...
  parser.add_argument('--leaderboard', action='store_true',
                      help='pass this flag if the submission is a leaderboard submission. It ensures the filename is in the right format')
  ...
  if args.leaderboard:
    if ('TADPOLE_Submission_Leaderboard_' not in forecastFile) or (not forecastFile.endswith('.csv')):
      raise ValueError(
          'Leaderboard submission filename is not in the correct format: TADPOLE_Submission_Leaderboard_TeamName.csv')
  else:
    if ('TADPOLE_Submission_' not in forecastFile) or (not forecastFile.endswith('.csv')):
      raise ValueError(
          'Submission filename is not in the correct format: TADPOLE_Submission_TeamName.csv.')
